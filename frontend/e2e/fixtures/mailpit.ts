...
// Same pattern as findInviteLink, for the forgot/reset-password flow's
// email — reset tokens are also stored hashed, only ever available in the
// email body.
export async function findPasswordResetLink(request: APIRequestContext, toEmail: string): Promise<string> {
  let messageId: string | undefined;
  ...
  await expect
    .poll(
      ...
      { message: `waiting for a password reset email to ${toEmail}`, timeout: 10_000 },
    )
    .toBeTruthy();
  ...
  const match = body.match(/\/reset-password\/[A-Za-z0-9_-]+/);
  if (!match) {
    throw new Error(`No /reset-password/:token link found in email body:\n${body}`);
  }
  return match[0];
}
